...
impl RoseEngineLathe {
    ...
    pub fn generate_tool_path(&mut self) {
        ...
        for i in 0..=self.config.resolution {
            ...
            let displacement = self
                .config
                .rosette
                .calculate_displacement(theta + self.config.phase, self.config.amplitude);
    ...
    pub fn to_svg(&self, filename: &str) -> Result<(), SpirographError> {
        ...
        svg::save(filename, &document).map_err(|e| SpirographError::ExportError(e.to_string()))
    }
...
mod tests {
    use super::*;
    use crate::rose_engine::{RoseEngineConfig, RosettePattern};
    ...
    fn test_generate_tool_path() {
        let config = RoseEngineConfig::new(RosettePattern::MultiLobe { lobes: 6 }, 1.0, 10.0, 100);
        let bit = CuttingBit::default();
        let mut lathe = RoseEngineLathe::new(config, bit).unwrap();

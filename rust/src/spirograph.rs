...
/// Common validation error for spirograph parameters
...
impl HorizontalSpirograph {
    /// Create a new horizontal spirograph
    pub fn new(
        outer_radius: f64,
        radius_ratio: f64,
        point_distance: f64,
    ...
    pub fn generate(&mut self) -> &Vec<Point2D> {
        let r = self.outer_radius * self.radius_ratio; // inner circle radius
...
impl VerticalSpirograph {
    /// Create a new vertical spirograph
    pub fn new(
        outer_radius: f64,
        radius_ratio: f64,
        point_distance: f64,
    ...
    pub fn generate(&mut self) -> &Vec<Point2D> {
        let r = self.outer_radius * self.radius_ratio;
        let R = self.outer_radius;
        let d = self.point_distance;
        ...
        for i in 0..total_points {
            ...
            // Base hypotrochoid
            let base_x = (R - r) * t.cos() + d * (((R - r) / r) * t).cos();
            let base_y = (R - r) * t.sin() - d * (((R - r) / r) * t).sin();
...
impl SphericalSpirograph {
    /// Create a new spherical spirograph
    pub fn new(
        outer_radius: f64,
        radius_ratio: f64,
        point_distance: f64,
    ...
    pub fn generate(&mut self) -> &Vec<Point3D> {
        ...
        let sphere_radius =
            (R * R + self.dome_height * self.dome_height) / (2.0 * self.dome_height);

        for i in 0..total_points {
            let t = 2.0 * PI * (i as f64) / (self.resolution as f64);

            // Base hypotrochoid in 2D
            let x_2d = (R - r) * t.cos() + d * (((R - r) / r) * t).cos();
            let y_2d = (R - r) * t.sin() - d * (((R - r) / r) * t).sin();

            self.points_2d.push(Point2D::new(x_2d, y_2d));
            ...
            let x_3d = x_2d * xy_scale;
            let y_3d = y_2d * xy_scale;
